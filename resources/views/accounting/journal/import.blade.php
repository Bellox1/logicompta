...
<div class="px-6 sm:px-12 py-10 w-full max-w-[1600px] mx-auto min-h-screen flex flex-col">
    ...
    <div class="mb-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8 animate-in fade-in slide-in-from-top duration-700">
        <div>
            <h1 class="text-4xl sm:text-5xl font-black text-slate-900 tracking-tighter uppercase leading-none">IMPORT JOURNAL</h1>
            <div class="flex items-center gap-3 mt-4">
                <span class="h-[2px] w-12 bg-slate-900"></span>
                <p class="text-[11px] text-slate-400 font-bold tracking-[0.4em] uppercase">Chargement massif via CSV (;)</p>
            </div>
        </div>
        <a href="{{ route('accounting.journal.index') }}" class="group flex items-center gap-4 px-8 py-4 bg-white border border-slate-100 rounded-none text-xs font-black uppercase text-slate-400 hover:text-black hover:border-black transition-all tracking-[0.2em] italic">
        ...
        <div id="error-alert" class="mb-12 p-8 bg-red-50 border-l-4 border-red-600 text-red-900 flex flex-col gap-8 shadow-sm animate-in fade-in zoom-in duration-500 relative">
            ...
            @if(session('needs_reindex'))
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6 pt-8 border-t border-red-100">
                    ...
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed max-w-xs">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 xl:gap-16 items-start">
            ...
            <div class="lg:col-span-8 flex flex-col gap-16 animate-in fade-in slide-in-from-left duration-700 delay-100">
                ...
                <div class="relative group h-[350px] sm:h-[400px]">
                    ...
                    <div id="drop-zone" class="w-full h-full border-2 border-dashed border-slate-200 rounded-none p-10 flex flex-col items-center justify-center text-center group-hover:bg-white group-hover:border-black transition-all duration-500 bg-slate-50/50 relative overflow-hidden">
                        <div id="icon-container" class="mb-8 relative transition-transform duration-500 group-hover:-translate-y-4">
                            <i data-lucide="upload-cloud" class="w-16 h-16 text-slate-200 group-hover:text-black transition-colors"></i>
                        </div>

                        <div class="space-y-4">
                            <h3 id="file-name" class="text-xl sm:text-2xl font-black text-slate-900 uppercase tracking-tighter">Déposez votre CSV</h3>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] opacity-60">ou cliquez pour choisir un fichier</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="w-full">
                    <div class="mb-10 flex items-center justify-between border-b-2 border-slate-900 pb-6">
                        <h3 class="text-xs font-black uppercase tracking-[0.4em] text-slate-900 italic leading-none">Illustration du format CSV</h3>
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest opacity-50 underline">Séparateur (;)</span>
                    </div>
                    
                    <div class="w-full">
                        <table class="w-full text-[10px] sm:text-xs font-mono border-collapse border-b border-slate-100">
                            <thead>
                                <tr class="text-slate-400 uppercase text-left border-b-2 border-slate-900">
                            ...
                            <tbody class="text-slate-900 font-bold divide-y divide-slate-100">
                                <tr class="group">
                                    <td class="py-6 pr-4">24/03/2026</td>
                                    <td class="py-6 pr-4 font-black tracking-tighter">PC001</td>
                                    <td class="py-6 pr-4">707</td>
                                    <td class="py-6 pr-4 italic font-medium hidden sm:table-cell text-slate-400">Vente de marchandises</td>
                    ...
                    <div class="mt-10 flex items-start gap-5 py-6 px-10 border-l-[4px] border-black bg-slate-50 italic">
                        <i data-lucide="info" class="w-6 h-6 text-black opacity-30 shrink-0"></i>
                        <p class="text-[11px] font-bold text-slate-600 uppercase tracking-widest leading-relaxed">
            ...
            <div class="lg:col-span-4 flex flex-col gap-10">
                <div class="bg-white border-2 border-slate-100 rounded-none p-10 flex flex-col h-full sticky top-32">
                    <h3 class="text-xs font-black uppercase tracking-[0.4em] text-slate-900 mb-10 pb-4 border-b-2 border-slate-900 italic">Colonnes</h3>
                    <div class="space-y-4 flex-grow">
                        @php 
                        $cols = ['DATE', 'Num PC (Réf)', 'JOURNAL', 'N° Compte', 'Libellé Compte', 'Libellé Op.', 'DÉBIT', 'CRÉDIT']; 
                        @endphp
                        @foreach($cols as $index => $col)
                        <div class="flex items-center justify-between p-4 border-b border-slate-50 group hover:border-black transition-all duration-300">
                            <span class="text-[11px] font-black text-slate-400 uppercase tracking-widest group-hover:text-black">{{ $col }}</span>
                            <i data-lucide="chevron-right" class="w-3 h-3 text-slate-200 group-hover:text-black opacity-0 group-hover:opacity-100 transition-all"></i>
                        </div>
                        @endforeach
                    </div>
