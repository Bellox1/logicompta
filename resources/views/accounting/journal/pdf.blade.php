...
<body class="p-8">
    <div class="no-print mb-6 flex justify-between items-center bg-gray-50 p-6 rounded-2xl border border-gray-200">
    ...
    <div class="mb-8 border-b-2 border-gray-900 pb-6 flex justify-between items-end">
        <div>
            <h1 class="text-3xl font-black uppercase tracking-tighter text-gray-900">JOURNAL COMPTABLE GÉNÉRAL</h1>
    ...
    <table class="w-full shadow-sm rounded-xl overflow-hidden border border-gray-200">
        <thead>
            <tr>
                ...
                @foreach ($entry->lines as $index => $line)
                    <tr class="{{ $loop->last ? 'entry-border' : '' }}">
                        @if ($index === 0)
                            ...
                            <td rowspan="{{ $entry->lines->count() }}" class="text-center font-black text-gray-900">
    ...
    <div class="mt-10 text-center text-[10px] text-gray-400 uppercase tracking-widest italic no-print opacity-50">
