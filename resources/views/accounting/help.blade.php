    ...
    <div>
            ...
            <div class="bg-card-bg border border-border p-8 shadow-sm transition-all hover:shadow-md group">
                ...
                <div class="space-y-3 text-sm leading-relaxed text-gray-600 dark:text-gray-400">
                    ...
                    <div class="flex gap-3"><b class="text-primary font-black shrink-0 w-16">Classe 2</b> <span>Actif
                            immobilisé (Matériel, Logiciels, etc.).</span></div>
                    <div class="flex gap-3"><b class="text-primary font-black shrink-0 w-16">Classe 3</b> <span>Stocks et
                            en-cours (Marchandises, Produits).</span></div>
                    <div class="flex gap-3"><b class="text-primary font-black shrink-0 w-16">Classe 4</b> <span>Tiers
                            (Clients, Fournisseurs, État).</span></div>
                    <div class="flex gap-3"><b class="text-primary font-black shrink-0 w-16">Classe 5</b> <span>Comptes de
                            trésorerie (Banques, Caisses).</span></div>
                    <div class="flex gap-3"><b class="text-primary font-black shrink-0 w-16">Classe 6</b> <span>Charges
                            (Dépenses liées à l'activité).</span></div>
                    <div class="flex gap-3"><b class="text-primary font-black shrink-0 w-16">Classe 7</b> <span>Produits
                            (Recettes et Revenus).</span></div>
                </div>
            </div>
        ...
        <div class="bg-primary text-white p-10 shadow-xl relative overflow-hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 text-sm">
                <div class="flex items-start gap-4">
                    ...
                    <p><b class="uppercase block text-xs opacity-70 mb-1">Équilibre</b> L'enregistrement est impossible si
                        le total débit ne correspond pas au crédit.</p>
                </div>
                <div class="flex items-start gap-4">
                    <div class="mt-1 w-2 h-2 bg-accent shrink-0"></div>
                    <p><b class="uppercase block text-xs opacity-70 mb-1">Période</b> Saisie flexible sur le mois en cours
                        avec 5 jours de battement sur le mois passé.</p>
                </div>
                <div class="flex items-start gap-4">
                    <div class="mt-1 w-2 h-2 bg-accent shrink-0"></div>
                    <p><b class="uppercase block text-xs opacity-70 mb-1">Intégrité</b> Les numéros de pièces sont
                        séquentiels et immuables une fois validés.</p>
                </div>
                <div class="flex items-start gap-4">
                    <div class="mt-1 w-2 h-2 bg-accent shrink-0"></div>
                    <p><b class="uppercase block text-xs opacity-70 mb-1">Exports</b> Chaque écran est optimisé pour être
                        exporté en format papier ou tableur.</p>
                </div>
            </div>
        </div>

        <!-- SECTION 4: RÉFÉRENTIEL PLAN COMPTABLE -->
        <div class="mt-16 bg-card-bg border border-border overflow-hidden shadow-2xl">
            <div
                class="bg-gray-100 dark:bg-black/40 p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between border-b border-border gap-4">
                <div>
                    <h2
                        class="text-xl font-black uppercase tracking-widest text-gray-800 dark:text-white flex items-center gap-3">
                        <i data-lucide="layout-list" class="w-6 h-6 text-primary"></i>
                        Référentiel des comptes (SYSCOHADA)
                    </h2>
                    <p class="text-xs text-gray-400 font-bold uppercase tracking-tighter mt-1">Consultez la liste officielle
                        des comptes disponibles</p>
                </div>
                <div class="relative max-w-xs w-full">
                    <i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400"></i>
                    <input type="text" id="searchAccount" placeholder="Rechercher par code ou libellé..."
            ...
            <div class="max-h-[600px] overflow-y-auto" id="account-list-container">
                @foreach ($accounts as $classe => $classAccounts)
                    <div class="classe-group">
                        <div class="bg-primary/5 px-6 py-2 border-y border-primary/10">
                            <span class="text-[11px] font-black uppercase text-primary tracking-widest">Classe
                                {{ $classe }}</span>
                        </div>
                            ...
                            <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                                @foreach ($classAccounts as $account)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors account-row"
                                        ...
                                        <td
                                            class="px-6 py-4 font-black text-primary tracking-widest w-32 border-r border-gray-50 dark:border-white/5">
                                            {{ $account->code_compte }}
                                        </td>
        ...
        <script>
            document.getElementById('searchAccount').addEventListener('input', function(e) {
                ...
                // Gérer la visibilité des groupes de classe
