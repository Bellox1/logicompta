...
<body class="bg-white p-4 md:p-8">
                ...
                <tbody>
                    @foreach($charges['groups'] as $group)
                        @foreach($group['accounts'] as $acc)
                            <tr><td class="font-bold">{{ $acc['code'] }}</td><td>{{ $acc['libelle'] }}</td><td class="text-right font-bold">{{ number_format($acc['montant'], 2, ',', ' ') }}</td></tr>
                        @endforeach
                        <tr class="group-total"><td colspan="2">Sous-total {{ $group['prefix'] }}</td><td class="text-right">{{ number_format($group['total'], 2, ',', ' ') }}</td></tr>
                    @endforeach
                </tbody>
                <tfoot>
    ...
    <div class="mt-8 border-4 border-primary p-10 text-center rounded-[2rem]">
        ...
        <div class="mt-4 inline-block px-6 py-2 border-2 {{ $profit >= 0 ? 'bg-green-100 border-green-200 text-green-700' : 'bg-red-100 border-red-200 text-red-700' }} rounded-xl font-bold uppercase text-[10px] tracking-widest">
