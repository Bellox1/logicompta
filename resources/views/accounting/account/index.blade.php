...
@section('content')
    ...
    <div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="bg-card-bg border border-border rounded-3xl shadow-sm overflow-hidden h-full flex flex-col">
                    ...
                    <div class="p-0 max-h-[800px] overflow-y-auto">
                        @if($allSousComptes->count() > 0)
                            <table class="w-full text-left">
                                <thead class="bg-slate-50 text-[10px] uppercase text-slate-400 font-bold tracking-widest border-b border-slate-100">
                                    <tr>
                                ...
                                <tbody class="divide-y divide-slate-50 text-[13px]">
                                    @foreach($allSousComptes as $sc)
                                        <tr class="hover:bg-slate-50/50 transition-colors">
                                            <td class="px-4 py-4 font-mono font-bold text-primary">{{ $sc->numero_sous_compte }}</td>
                                            <td class="px-4 py-4 font-semibold text-slate-700">
                                                {{ $sc->libelle }}
                                                <div class="text-[9px] text-slate-400 font-bold uppercase mt-1">Parent: {{ $sc->account->code_compte }}</div>
                                            </td>
                        ...
                        @else
                            <div class="p-16 text-center">
                                ...
                                <p class="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-wider">Configurez vos comptes auxiliaires</p>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div id="subAccountModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] hidden items-center justify-center p-4">
            <div class="bg-card-bg rounded-3xl shadow-2xl max-w-md w-full relative overflow-hidden animate-in fade-in zoom-in duration-200 border border-border">
                ...
                <form action="{{ route('accounting.account.store_sous_compte') }}" method="POST" class="p-8">
                    ...
                    <div class="mb-6">
                        <label class="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Compte de rattachement</label>
                        <div id="modal_compte_label" class="p-4 bg-slate-50 border border-slate-100 rounded-lg text-sm font-bold text-slate-600"></div>
                    </div>
                    
                    <div class="mb-6">
                        <label class="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Numéro personnalisé</label>
                        ...
                        <p class="text-[10px] text-slate-400 mt-2 font-medium">Doit être unique dans votre comptabilité.</p>
                    </div>

                    <div class="mb-8">
                        <label class="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Libellé descriptif</label>
                    ...
                    <div class="flex flex-col gap-3">
                        <button type="submit" 
                            class="w-full bg-primary text-white py-4 rounded-lg font-bold hover:opacity-90 transition-all shadow-sm">
                            Créer le sous-compte
                        </button>
                        <button type="button" onclick="closeSubAccountModal()" 
                            class="w-full py-3 font-bold text-slate-400 text-xs uppercase tracking-widest hover:text-slate-600 transition-all">
                            Annuler
                        </button>
                    </div>
                </form>
        ...
        <div id="editSubAccountModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] hidden items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-2xl max-w-md w-full relative overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <form id="editForm" method="POST" class="p-8">
                    ...
                    <div class="mb-6">
                        <label class="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Numéro de Sous-compte</label>
                        <input type="text" name="numero_sous_compte" id="edit_numero_sous_compte" required 
                            class="w-full bg-white border border-slate-200 p-4 text-sm font-bold rounded-lg outline-none focus:border-primary transition-all">
                    </div>

                    <div class="mb-8">
                        <label class="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Libellé du Sous-compte</label>
