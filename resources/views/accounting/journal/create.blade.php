            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
                        ...
                        <select name="journal_id" required
                            ...
                            @foreach ($journals as $journal)
                                <option value="{{ $journal->id }}"
                                    {{ old('journal_id') == $journal->id ? 'selected' : '' }}>{{ $journal->code }} -
                                    {{ $journal->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="min-w-0">
                    <label
                        ...
                        général</label>
                    <input type="text" name="libelle" value="{{ old('libelle') }}" placeholder="Ex: Règlement..."
                        required
                        class="w-full max-w-full bg-gray-50 border border-border rounded-xl px-3 py-2 md:px-4 md:py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all text-sm md:text-base">
                </div>
            </div>
                ...
                <table class="w-full border-collapse min-w-[800px]">
                    <thead>
                        <tr class="bg-white text-gray-500 border-b border-border">
                            <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-left" style="width: 35%;">
                                Compte</th>
                            <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-left" style="width: 15%;">
                                Débit</th>
                            <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-left" style="width: 15%;">
                                Crédit</th>
                    ...
                    <tbody id="lines-body" class="divide-y divide-gray-100">
                        ...
                        @foreach ($oldLines as $index => $oldLine)
                            <tr class="line-row hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <input type="text" name="lines[{{ $index }}][libelle]"
                                        value="{{ $oldLine['libelle'] ?? '' }}"
                    ...
                    <div class="flex flex-wrap gap-4 md:gap-8">
                        ...
                        <div id="balance-container">
                            <div class="text-[10px] uppercase font-bold text-gray-400 tracking-widest mb-1">État
                                d'Équilibre</div>
                            <div id="balance-status" class="text-sm font-bold text-red-600 italic">Déséquilibre: 0,00 F
                            </div>
                        </div>
                    </div>
                ...
                if (balanceStatusEl) {
                    if (absDiff < 0.001 && totalDebit > 0) {
                        ...
                        balanceStatusEl.innerHTML =
                            `${message}: <span class="magical-balance">${absDiff.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span> F`;
            ...
            if (addLineBtn) {
                addLineBtn.addEventListener('click', () => {
                    ...
                    deleteCell.innerHTML =
                        '<button type="button" class="text-red-400 hover:text-red-600 transition-colors p-1" onclick="this.closest(\'tr\').remove(); calculate();"><i data-lucide="trash-2" class="w-5 h-5"></i></button>';
