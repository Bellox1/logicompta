...
<div class="max-w-[800px] mx-auto">
    ...
    <div class="bg-card-bg border border-border rounded-none shadow-sm overflow-hidden p-8">
        <form action="{{ route('accounting.journals-settings.store') }}" method="POST" class="space-y-10">
            @csrf
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', 'Journal de ') }}" placeholder="Ex: Journal de Caisse" required
                ...
                <div>
                    ...
                    <textarea name="description" placeholder="Ex: Opérations en espèces uniquement..." rows="4"
                        class="w-full bg-white border border-border px-6 py-4 text-sm font-bold text-slate-700 outline-none focus:border-primary transition-all rounded-xl shadow-sm resize-y">{{ old('description') }}</textarea>
                </div>
            </div>
