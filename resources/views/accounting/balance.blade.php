...
<div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 no-print">
    <div>
        ...
        <p class="text-sm text-text-secondary mt-1 font-bold italic">Vérification de l'équilibre arithmétique de vos comptes</p>
    </div>
...
<div class="mb-10 no-print">
    <form action="{{ request()->url() }}" method="GET" class="mb-10 grid grid-cols-1 md:flex md:flex-row md:items-end gap-5 bg-card-bg border border-border p-6 rounded-3xl shadow-sm no-print">
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="px-8 py-3 bg-primary text-white text-xs font-black uppercase tracking-widest hover:bg-primary-light hover:-translate-y-0.5 transition-all shadow-lg rounded-xl flex items-center gap-2">
                <i data-lucide="refresh-cw" class="w-4 h-4"></i> Actualiser
            </button>
            @if(request()->hasAny(['start_date', 'end_date']))
                <a href="{{ request()->url() }}" class="px-6 py-3 bg-white text-text-secondary text-xs font-black uppercase tracking-widest hover:bg-slate-50 transition-all rounded-xl flex items-center gap-2 border border-border shadow-sm">
...
<div class="bg-card-bg border border-border rounded-[2.5rem] shadow-sm overflow-hidden mb-12" id="balance-table">
    <div class="bg-white/50 border-b border-border px-8 py-8 text-center">
        <h2 class="text-xl font-black text-text-main uppercase tracking-[0.2em] italic">État de la Balance au {{ date('d/m/Y') }}</h2>
    </div>
        ...
        <table class="w-full border-collapse min-w-[1000px]">
            <thead>
                <tr class="text-text-secondary border-b border-border font-black italic">
                    <th rowspan="2" class="px-6 py-4 text-[10px] uppercase tracking-widest text-left" style="width: 140px;">Compte</th>
                    <th rowspan="2" class="px-6 py-4 text-[10px] uppercase tracking-widest text-left">Intitulé</th>
                    <th colspan="2" class="px-6 py-3 text-[10px] uppercase tracking-widest text-center border-l border-border">Flux de la période</th>
                    <th colspan="2" class="px-6 py-3 text-[10px] uppercase tracking-widest text-center border-l border-border">Solde final</th>
                </tr>
                <tr class="text-text-secondary border-b border-border font-black italic">
                    <th class="px-6 py-2 text-right text-[10px] uppercase border-l border-border">Débit</th>
                    <th class="px-6 py-2 text-right text-[10px] uppercase">Crédit</th>
                    <th class="px-6 py-2 text-right text-[10px] uppercase border-l border-border">Débit</th>
                    <th class="px-6 py-2 text-right text-[10px] uppercase">Crédit</th>
                </tr>
            </thead>
                ...
                @forelse($balanceData as $classId => $class)
                    ...
                    @foreach($class['groups'] as $prefix => $group)
                        ...
                        @foreach($group['accounts'] as $acc)
                            <tr class="border-b border-border hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                <td class="px-4 py-3 font-mono font-bold text-text-main border-r border-border italic">{{ $acc['code'] }}</td>
                                <td class="px-4 py-3 text-text-secondary border-r border-slate-200 dark:border-white/5 font-medium uppercase">{{ $acc['libelle'] }}</td>
                                
                                <td class="px-4 py-3 text-right font-black text-text-main border-r border-slate-200 dark:border-white/5 whitespace-nowrap">
                                ...
                                <td class="px-4 py-3 text-right font-black text-primary dark:text-primary-light border-r border-slate-200 dark:border-white/5 whitespace-nowrap bg-primary/5">
