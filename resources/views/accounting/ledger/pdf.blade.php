...
<head>
    ...
    <title>Grand Livre - {{ $user->entreprise->name ?? 'COMPTAFIQ' }}</title>
    <!-- Font: Arial -->
...
<body class="bg-white p-4 md:p-8">
    ...
    @foreach ($data as $account)
        <div class="mb-10 {{ !$loop->last ? 'page-break' : '' }}">
            <div
                class="bg-slate-50 border-x border-t border-slate-300 p-3 flex flex-col md:flex-row md:items-center justify-between gap-2">
                <span class="text-sm font-black uppercase text-primary">{{ $account->code_compte }} -
                    {{ $account->libelle }}</span>
                <span class="text-[10px] font-bold italic text-slate-400">Compte de classe
                    {{ substr($account->code_compte, 0, 1) }}</span>
            </div>
                ...
                <table class="w-full min-w-[800px]">
                    ...
                    <tbody>
                        ...
                        @foreach ($account->entryLines as $line)
                            ...
                            <tr>
                                <td class="text-center">{{ \Carbon\Carbon::parse($line->entry->date)->format('d/m/Y') }}
                                </td>
                    ...
                    <tfoot>
                        <tr class="bg-slate-50 font-bold">
                            ...
                            <td class="text-right">{{ number_format($account->entryLines->sum('debit'), 2, ',', ' ') }}
                            </td>
                            <td class="text-right">
                                {{ number_format($account->entryLines->sum('credit'), 2, ',', ' ') }}</td>
                        </tr>
                        <tr class="font-black bg-white">
                            <td colspan="3" class="text-right uppercase">Solde Net
                                {{ $runningSolde >= 0 ? 'débité' : 'crédité' }}</td>
