...
<head>
    <meta charset="UTF-8">
    <title>Bilan Financier - {{ $user->entreprise->name ?? 'COMPTAFIQ' }}</title>
    <!-- Font: Arial -->
...
<body class="bg-white p-4 md:p-8">
        ...
        <div class="flex-1 overflow-x-auto shadow-sm rounded-xl border border-slate-200">
            <table class="w-full min-w-[500px]">
                <thead>
                    <tr>
                        <th colspan="2"
                            class="text-lg py-3 bg-primary text-white text-left font-black tracking-widest">ACTIF
                            (IMMOBILISATIONS & DISPONIBILITÉS)</th>
                    </tr>
                    <tr>
                        <th>POSTES</th>
                        <th style="width: 100px; text-align: right;">MONTANT NET</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($actif as $item)
                        <tr>
                            <td>{{ $item['libelle'] }}</td>
                            <td class="text-right font-bold">{{ number_format($item['solde'], 2, ',', ' ') }}</td>
                        </tr>
                    @endforeach
                ...
                <tfoot>
                    <tr class="total-row">
                        <td class="text-right uppercase">TOTAL GÉNÉRAL DE L'ACTIF</td>
                        <td class="text-right text-lg">{{ number_format($actif->sum('solde'), 2, ',', ' ') }}</td>
                    </tr>
                </tfoot>
        </div>
        ...
        <div class="flex-1 overflow-x-auto shadow-sm rounded-xl border border-slate-200 mt-6 xl:mt-0">
            <table class="w-full min-w-[500px]">
                <thead>
                    <tr>
                        <th colspan="2"
                            class="text-lg py-3 bg-primary text-white text-left font-black tracking-widest">PASSIF
                            (CAPITAUX PROPRES & DETTES)</th>
                    </tr>
                    <tr>
                        <th>POSTES</th>
                        <th style="width: 100px; text-align: right;">VALEURS</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($passif as $item)
                        <tr>
                            <td class="{{ isset($item['is_resultat']) ? 'italic text-primary font-black' : '' }}">
                                {{ $item['libelle'] }}</td>
                            <td
                                class="text-right font-bold {{ isset($item['is_resultat']) ? ($item['solde'] >= 0 ? 'text-green-700' : 'text-red-700') : '' }}">
                ...
                <tfoot>
                    <tr class="total-row">
                        <td class="text-right uppercase">TOTAL GÉNÉRAL DU PASSIF</td>
                        <td class="text-right text-lg">{{ number_format($passif->sum('solde'), 2, ',', ' ') }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    @if ($diff > 0.01)
        <div
            class="mt-6 text-center text-red-600 font-bold border-4 border-red-600 p-4 rounded-xl uppercase tracking-widest no-print">
